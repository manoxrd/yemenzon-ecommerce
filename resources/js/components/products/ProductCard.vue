...
<!-- {{ item.reviews }} -->
  <div class="relative h-56 overflow-hidden bg-muted/60 dark:bg-muted/30">
      ...
      <div v-if="false"
        ...
        <!-- product's discount time -->
      </div>
    </div>

    <!-- Content -->
    <div class="flex grow flex-col p-5">
      <span class="mb-1.5 text-xs font-bold tracking-widest text-muted-foreground uppercase">{{ product.category?.name
        }}</span>
      <h3
      ...
      <div class="mb-5 flex products-center justify-between">
        <span class="text-xl font-black tracking-tight text-foreground">
          <ProductPrice :price="product.price" />
        </span>
        <StarReview variant="mini" :size="14" :avg_rating="product.rating" :reviews_count="product.reviews_count" />
      </div>
